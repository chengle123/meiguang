  ...
  <div class="login">
    <div class="loginFrame">
        ...
        <div class="text-right">
            ...
            <!--<el-button type="text" @click="shop();">购买</el-button>-->
        </div>
    ...
    <span id="version" style="bottom: 30px;">版本：V {{ version }}</span>
    <span id="version" style="bottom: 10px;">设备号：{{ equipmentId }}</span>
  </div>
</template>
...
export default {
    ...
    mounted(){
        var _this = this;
        setTimeout(()=>{
            this.$http.post('http://localhost:8990/equipmentId').then(function (data) {
                if(data && data.data.result === 'success'){
                    _this.equipmentId = data.data.data.equipmentId;
                }
            })
        },100)
    },
    methods: {
        ...
        shop(){
            window.open('http://transaction.sansantao.com/', '_blank');
        }
    }
}
</script>
